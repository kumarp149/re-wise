...
#define __RESTORE_FLAGBIT_DELETE__ 2

#define __RESTORE_FLAGS__ \
  X("--help","-h" , "help","display this usage", (__RESTORE_FLAGBIT_HELP__)) \
  X("--delete","-d" , "delete","force delete the path if absent at HEAD", (__RESTORE_FLAGBIT_DELETE__)) \

#define __RESTORE_ARGS__ \
  X(true,"--path","-p","path","restore the specific paths from the HEAD (regex unsupported)",(1<<20)) \
